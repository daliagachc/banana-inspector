...
class BananaPlotCut2(pg.GraphicsLayoutWidget):
...
class BananaPlotNodeCut2(CtrlNodeTree):
    """plots dndlDp spectra data """
    nodeName = "BananaPlotNodeCut2"
    uiTemplate = [
        # {'name': 't1', 'type': 'float', 'value': 1e9},
        # {'name': 't2', 'type': 'float', 'value': 2e9},
        # {'name': 't3', 'type': 'float', 'value': 2},
        {'name': 'dock', 'type': 'str', 'value': nodeName},
        {'name': 'title', 'type': 'str', 'value': 'log( dN /dlog(Dp) )'},
        {
            'name' : 'log10',
            'type' : 'bool',
            'value': False,
    ...
    def process(self, dataIn, t1, t2, display=True):
        ...
        title = self.ctrls['title']

        if duck_name not in dock_area.findAll()[1].keys():
            bp = BananaPlotCut2(None)
            bp.plot_item.setTitle(title)
        ...
        if self.bp is not None:
            ...
            self.bp.plot_item.setTitle(title)
